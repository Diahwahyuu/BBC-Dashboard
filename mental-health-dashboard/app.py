...
def load_data():
    ...
    DATA_PATH = os.path.join(
        CURRENT_DIR, "data", "data_dengan_topik_diperbarui.csv"
    )
...
# 1. Ekstrak nomor topik secara aman
...
# 2. Definisikan path absolut ke file wordcloud
image_filename = f"wordcloud_topik_{topic_number}.png"
...
with col_wc:
  ...
  if os.path.exists(image_path):
    ...
    st.warning(
        f"File `{image_filename}` tidak ditemukan di `{ASSETS_DIR}`. Pastikan"
        f" nama file adalah `wordcloud_topik_{topic_number}.png`."
    )
